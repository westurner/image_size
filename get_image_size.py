...
def _get_image_metadata(fileobj, file_path=None, size=None):
    ...
    if (size >= 10) and data[:6] in ('GIF87a', 'GIF89a'):
        ...
        try:
            while (b and ord(b) != 0xDA):
                ...
                if (ord(b) >= 0xC0 and ord(b) <= 0xC3):
                    fileobj.read(3)
                    h, w = struct.unpack(">HH", fileobj.read(4))
                    break
                else:
                    fileobj.read(
                        int(struct.unpack(">H", fileobj.read(2))[0]) - 2)
                b = fileobj.read(1)
            width = int(w)
            height = int(h)
        except struct.error:
        ...
        try:
            countSize = 2
            fileobj.seek(ifdOffset)
            ec = fileobj.read(countSize)
            ...
            for i in range(ifdEntryCount):
                ...
                if(tag == 256 or tag == 257):
                    ...
                    fileobj.seek(entryOffset + 8)
                    value = fileobj.read(typeSize)
...
class Test_get_image_size(unittest.TestCase):
    ...
    def test_get_image_metadata__Image__from_file(self):
        img = self.data[0]
        output = Image.from_file(img['path'])
        self.assertTrue(output)
        self.assertEqual(output.path, img['path'])
        self.assertEqual(output.width, img['width'])
        self.assertEqual(output.height, img['height'])
    ...
    def test_get_image_metadata__fileobj(self):
        img = self.data[0]
        with open(img['path'], 'rb') as fileobj:
            output = get_image_metadata(fileobj, file_path=img['path'])
            self.assertTrue(output)
            self.assertEqual(output.path, img['path'])
            self.assertEqual(output.width, img['width'])
            self.assertEqual(output.height, img['height'])
